...
- Cause classification (`classifyCause` in `src/cause.ts`) is an ordered first-match keyword scan (priority: strike → weather → technical → operational → construction → unknown). `operational` is the catch-all for these notices: it covers the generic umbrella term `betriebsbedingt` as well as explicit staff/sickness wording (`personalmangel`, `krankheit…`). Extend it by adding keywords to a group or a new `{ cause, keywords }` entry; a rising share of `unknown` means the keyword lists need extending.
