...
- Cause classification (`classifyCause` in `src/cause.ts`) is an ordered first-match keyword scan (priority: strike → weather → technical → operational → disruption → construction → unknown). `operational` is the catch-all for the staffing-flavored notices: it covers the generic umbrella term `betriebsbedingt` as well as explicit staff/sickness wording (`personalmangel`, `krankheit…`). `disruption` is the separate bucket for a bare `Betriebsstörung` (an unspecified acute disruption) — deliberately below `technical` (so a named fault wins) and separate from `operational` (so it never pollutes the staffing signal). Extend it by adding keywords to a group or a new `{ cause, keywords }` entry; a rising share of `unknown` means the keyword lists need extending.
