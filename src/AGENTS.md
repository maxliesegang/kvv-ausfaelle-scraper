...
- Cause classification (`classifyCause` in `src/cause.ts`) is an ordered first-match keyword scan (priority: strike → weather → technical → personnel → operational → construction → unknown). `personnel` is reserved for explicit staff/sickness wording (`personalmangel`, `krankheit…`); the generic `betriebsbedingt` maps to `operational`. Extend it by adding keywords to a group or a new `{ cause, keywords }` entry; a rising share of `unknown` means the keyword lists need extending.
