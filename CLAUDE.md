...
Relevance keeps all true cancellation notices and never vetoes by cause — construction-only notices are retained, not dropped. Instead, each cancellation carries a best-effort `cause` field (`CancellationCause`: `operational | strike | technical | disruption | weather | construction | unknown`) computed by `classifyCause` (`src/cause.ts`), an ordered first-match keyword classifier. `operational` is the staffing-flavored catch-all: it covers both the generic `betriebsbedingt` (KVV's umbrella euphemism) and explicit staff/sickness wording (`personalmangel`, `krankheit…`). `disruption` is a separate bucket for a bare `Betriebsstörung` — an unspecified acute disruption KVV reports without naming a cause; it sits below `technical` (a named fault still wins) and apart from `operational` so it never pollutes the staffing signal.
