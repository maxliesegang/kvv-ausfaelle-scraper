...
2. **Fetch detail pages** (`src/workflow.ts`) — for each relevant item, fetch the HTML, re-check relevance, and skip articles younger than 1 hour (`MIN_ARTICLE_AGE_MS`). Uses `Promise.allSettled` so one bad article never aborts the run. Every relevant article that passes the relevance check is also archived as plain text (`src/article-archive.ts` → `docs/<fahrplan-year>/articles/<detailID>.txt`) _before_ the age/parse decision, so even too-young or unparsable notices leave a record. The file scopes to the page's `<main>` region and carries only stable content (a `Quelle:`/`Stand:`/`Titel:`/`Datum:` header + article body, no per-run timestamp), so an unchanged article yields a byte-identical file — turning the committed `docs/` git history into a timeline of KVV's in-place edits. `Titel:` and `Datum:` record the RSS item's title and publication date — the inputs the relevance and age gates judged the article by, which the detail page itself does not contain — so a gate decision stays replayable; both read back as absent on archives written before the fields existed (see `docs/AGENTS.md`). Archiving is best-effort and never fails the run.
3. **Parse trips** (`src/parser/**`) — extract individual structured trip cancellations from the HTML, and classify the article-level cause (`src/cause.ts`) which is stamped onto every trip. `src/parser/article-corrections.ts` repairs corrupt published values (e.g. a malformed clock time) in the article _text_ before parsing, scoped to a single `detailID` and admitted only when GTFS resolves the value uniquely — so live runs, tests and the reparse tooling all yield the same trips, while the text archive keeps KVV's original wording.
4. **Store + index** (`src/storage.ts`, `src/site-index.ts`) — bucket by line, deduplicate, **reconcile** (drop stored trips whose re-fetched source article no longer lists them — KVV edits detail pages in place, sometimes without bumping `Stand`, so trips can silently vanish), write `docs/<fahrplan-year>/<line>.json`, and regenerate root/year HTML + JSON index pages. Reconciliation only prunes entries whose `sourceUrl` was successfully re-fetched this run, so a transient fetch failure never deletes stored data, and it is **forward-looking only** (see invariants).
...
- **Reconciliation is forward-looking; the past is append-only.** KVV detail pages are rolling "still upcoming" lists — during a long disruption the article gets rewritten to drop trips that have already departed, sometimes without bumping `Stand`. So a stored trip is prunable only while its departure is still ahead (un-listing it is a real retraction); **a departed trip is never pruned**, because an article dropping it is garbage collection and says nothing about whether it ran. Accepted cost: a past entry KVV genuinely retracts stays stored. `src/storage.ts` and `scripts/reparse-archives.ts` apply the same rule so the tooling and the live scraper converge instead of undoing each other, and every retention is logged (`= kept …`) so the rule is visible in a run rather than silent. Trip times are Europe/Berlin wall clock and must be compared via `getBerlinWallClockMs` (`src/utils/berlin-time.ts`), never a naive `new Date()`, which would resolve in the runner's zone (UTC in CI).
