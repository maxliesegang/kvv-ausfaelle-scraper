...
- Reparse the text archive vs stored data (read-only report): `npm run reparse-archives [-- --year=N --verbose]`; add `--write` to backfill `cause`/`causeKeyword` on archived trips
