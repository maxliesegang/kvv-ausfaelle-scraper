...
Relevance keeps all true cancellation notices and never vetoes by cause — construction-only notices are retained, not dropped. Instead, each cancellation carries a best-effort `cause` field (`CancellationCause`: `personnel | operational | strike | technical | weather | construction | unknown`) computed by `classifyCause` (`src/cause.ts`), an ordered first-match keyword classifier. `personnel` is reserved for explicit staff/sickness wording; the generic `betriebsbedingt` (KVV's umbrella term for these notices) maps to `operational`.
