...
4. **Store + index** (`src/storage.ts`, `src/site-index.ts`) — bucket by line, deduplicate, write `docs/<year>/<line>.json`, and regenerate root/year HTML + JSON index pages.

Relevance keeps all true cancellation notices and never vetoes by cause — construction-only notices are retained, not dropped. Instead, each cancellation carries a best-effort `cause` field (`CancellationCause`: `personnel | strike | technical | weather | construction | unknown`) computed by `classifyCause` (`src/cause.ts`), an ordered first-match keyword classifier.

Train-number → line mapping (`src/train-lines.ts`, `src/train-line-definitions/`) is learned over time: while parsing, observed train/line pairs are recorded (`src/train-line-observations.ts`) and persisted, with fallback matching when no exact mapping exists.
